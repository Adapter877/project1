...
if(isset($_POST['update'])){
   ...
   $exam_link = $_POST['exam_link'];  // เพิ่มการรับค่า exam_link
   $exam_link = filter_var($exam_link, FILTER_SANITIZE_URL);  // กรองค่า exam_link
   
   // เพิ่ม exam_link ลงใน query เพื่ออัปเดต
   $update_content = $conn->prepare("UPDATE `content` SET title = ?, description = ?, status = ?, exam_link = ? WHERE id = ?");
   $update_content->execute([$title, $description, $status, $exam_link, $video_id]);
...
<section class="video-form">
   ...
   <h1 class="heading">อัปเดตเนื้อหา</h1>
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>อัปเดตเพลย์ลิสต์</p>
      <select name="playlist" class="box">
         <option value="<?= $fecth_videos['playlist_id']; ?>" selected>--เลือกเพลย์ลิสต์</option>
         <?php
      ...
      <p>อัปเดตภาพขนาดย่อ</p>
      <input type="file" name="thumb" accept="image/*" class="box">
      <video src="../uploaded_files/<?= $fecth_videos['video']; ?>" controls></video>
      <p>อัปเดตวิดีโอ</p>
      <input type="file" name="video" accept="video/*" class="box">
      <input type="submit" value="อัปเดตเนื้อหา" name="update" class="btn">
      <div class="flex-btn">
         <a href="view_content.php?get_id=<?= $video_id; ?>" class="option-btn">ดูเนื้อหา</a>
         <input type="submit" value="ลบเนื้อหา" name="delete_video" class="delete-btn">
      </div>
   </form>
   <?php
         }
      }else{
         echo '<p class="empty">ไม่พบวิดีโอ! <a href="add_content.php" class="btn" style="margin-top: 1.5rem;">add videos</a></p>';
      }
   ?>
